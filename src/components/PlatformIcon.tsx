...
import bigoLiveMark from '@/assets/platform-bigo-live.png';

// A platform declares its mark ONE of two ways.
//
// `Icon` — the rule. A monochrome 24x24 path (Simple Icons, Font Awesome, or
// traced in-house below) that tints to any color via the `color` prop.
//
// `image` — TL.POLISH.1d's documented exception, for a brand whose mark cannot
// survive being flattened to one color. Bigo Live is the first: its mascot
// reads only because a dark outline separates the white body from the cyan
// crest, so any monochrome trace collapses into a blob (three attempts proved
// it), and it is in neither Simple Icons nor Font Awesome. An image mark is
// full-color and therefore does NOT follow the icon-row color mode or the
...
// TL.POLISH.1d: renders an `image` mark. It paints the bitmap INSIDE the same
// 24x24 <svg> shell a path mark uses, so it is a drop-in at the DOM level —
// one <svg> of identical size, inline-baseline alignment, and no rounding of
// its own (the source is trimmed to transparency, so the chip's circle shows
// through). That parity matters beyond looks: `.lb-social svg { width:22px }`
// in index.css re-sizes the leading icon, and every such rule keeps applying
...
// Every PATH mark above is a single-color monochrome glyph tinted via `color`,
// so the `color` field doubles as the glyph-luminance hint. (TL.POLISH.1d:
// image marks are exempt — PlatformIcon ignores the returned tint for them, so
// they carry their own contrast. Bigo Live's mascot does: a dark outline holds
// it on light chips, a white body on dark ones.) Catalog audit (52 platforms):
...
//     (1.37). Bigo Live #00FFEE (1.27) still measures here but is an image
//     mark, so the rescue is computed and then discarded at render.
